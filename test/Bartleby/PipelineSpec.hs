...
spec :: Spec
spec = do
  beforeAll_ cleanup $ afterAll_ cleanup $
  ...
  let altRoot = "test/fixtures/pipeline/gophermap-name"
      altCleanup = do
        removePathForcibly (altRoot </> "catalog")
        removePathForcibly (altRoot </> "catalog.tmp")
  beforeAll_ altCleanup $ afterAll_ altCleanup $
    describe "gophermap_filename override" $ do

      it "writes 'gophermap' instead of '.gophermap' when overridden" $ do
        code <- Pipeline.run altRoot
        code `shouldBe` ExitSuccess
        doesFileExist (altRoot </> "catalog" </> "gophermap")
          `shouldReturn` True

      it "does not write '.gophermap' when overridden" $ do
        _ <- Pipeline.run altRoot
        doesFileExist (altRoot </> "catalog" </> ".gophermap")
          `shouldReturn` False
