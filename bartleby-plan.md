...
| `include_dotfiles`   | no       | `false`       | When `true`, file and directory names starting with `.` are catalogued instead of skipped — useful for surfacing `.plan`, `.signature`, etc. No carve-outs: if you enable this on a versioned library, `.git/` and friends will be walked too. See the Gopher item type section for how to type extensionless dotfiles. |
